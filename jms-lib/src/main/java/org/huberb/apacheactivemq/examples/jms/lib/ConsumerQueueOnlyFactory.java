...
import org.huberb.apacheactivemq.examples.jms.lib.AutoCloseableSupport.AutoCloseableMessageConsumer;
 ...
 * @author pi
 */
public class ConsumerQueueOnlyFactory {
     ...
     */
    public ConsumerQueueOnlyFactory(ConnectionFactory connectionFactory) {
     ...
     * @return
     */
    public Optional<Message> consumeQueueMessage(
        ...
        consumeQueueMessage(m, queueName, jmsMessageSelector, maxReceiveCount, maxWaittimeSeconds, messageConsumer);
    ...
    public void consumeQueueMessage(
        ...
        try (final AutoCloseableConnection autocloseableConnection = new AutoCloseableConnection(connectionFactory.createConnection())) {
            ...
            try (final AutoCloseableSession autoCloseableSession = new AutoCloseableSession(connection.createSession(transacted, acknowledgeMode))) {
                ...
                try (final AutoCloseableMessageConsumer autoCloseableMessageConsumer = new AutoCloseableMessageConsumer(session.createConsumer(destination, jmsMessageSelector, false))) {
